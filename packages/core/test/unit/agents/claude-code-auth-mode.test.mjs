...
import { resolveClaudeCodeGatewayAuthMode } from "@agentrouter/core/agents/claude-code/auth-mode.ts";
...
test("#1779 auto auth does not infer working federation from a CLI version", (t) => {
  ...
  assert.equal(resolveClaudeCodeGatewayAuthMode({ env: { AR_CLAUDE_CODE_AUTH_MODE: "auto" } }), "api-key-helper");
...
test("#1779 explicit federation and helper preferences remain available", () => {
  for (const mode of ["wif", "api-key-helper"]) {
    assert.equal(resolveClaudeCodeGatewayAuthMode({ env: { AR_CLAUDE_CODE_AUTH_MODE: mode } }), mode);
  }
});
