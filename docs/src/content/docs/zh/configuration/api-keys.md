...
pageTitle: API 密钥
eyebrow: 详细配置
lead: 管理客户端访问 AgentRouter 网关时使用的 API Key，并为每个 Key 设置过期时间和本地限额。
---
...
| 过期 | 当前 Key 的过期时间。过期后客户端不能继续使用这个 Key 访问 AgentRouter。 |
...
| 请现在复制保存这个密钥，之后可能不会再次完整显示。 | 提醒你立即复制 Key。关闭弹窗后，AgentRouter 不会再次展示完整 Key。 |
