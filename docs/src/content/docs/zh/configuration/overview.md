...
pageTitle: 概览仪表盘
eyebrow: 详细配置
lead: 自定义 AgentRouter 首页仪表盘，查看系统状态、账户余额、请求量、令牌、成本和模型分布。
---
...
| 汇报或分享用量 | AI 用量年报、AgentRouter 路由图、模型排行榜、消费小票等分享卡片 |
...
| 分享卡片 | AI 用量年报、AgentRouter 路由图、模型排行榜、AI 燃料仪表、Token 日历海报、消费小票 | `1:4` | 卡片 | 卡片 |
...
| AgentRouter 路由图 | `share-route-map` | 客户端到供应商 / 模型的主要路由关系，以及客户端、供应商、模型数量。 |
...
| 现象 | 可能原因 | 处理方式 |
| --- | --- | --- |
| 请求、令牌或成本为 0 | 当前时间范围内没有经过 AgentRouter 的请求，或用量捕获尚未记录。 | 切换到 `24h` / `7d`，确认客户端请求确实走 AgentRouter。 |
...
| 分布图没有数据 | 请求日志里缺少模型、供应商或 token 信息。 | 确认请求经过 AgentRouter，并检查上游响应是否返回 token usage。 |
