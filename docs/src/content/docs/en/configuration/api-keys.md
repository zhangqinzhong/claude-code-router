...
lead: Manage API keys that clients use to access the AgentRouter gateway, with expiration and local limits.
---
...
| Expires | Expiration time. After expiration, clients can no longer use the key to access AgentRouter. |
...
| Copy this key now. It may not be shown again. | Reminder to copy the key immediately because AgentRouter will not show it again after the dialog closes. |
