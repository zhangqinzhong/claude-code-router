...
| Config name | All | Identifies the config in CCR. Desktop commands use `agentrouter <config-name>`; CLI commands use `ccr <config-name>`. Names can contain spaces; copied commands are quoted automatically. |
...
Grok CLI profiles are fixed to **Only opened from CCR** and **CLI only**. After saving, run the profile command: desktop `agentrouter "Grok - Work"`, or CLI `ccr "Grok - Work"`. Inside Grok CLI, use `/model` to switch among the provider and Fusion models returned by CCR.
